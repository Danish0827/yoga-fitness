  ...
  useEffect(() => {
    const fetchGalleryData = async () => {
      const response = await fetch(
        `${process.env.ADMINURL}/api/allblogs?visibility=public`
      );
      const results = await response.json();
      setLatestBlog(results.blogs);
          ...
          <div className="px-5">
            ...
            {latestBlog?.slice(0, 4)?.map((item: any, index: any) => (
              <div key={index} className="bg-[#f4f4f4] p-3 rounded-lg mb-2">
                ...
                <Link href={`/blog/${item.id}`} className="flex flex-wrap">
                  <div className="max-h-[120px] w-1/3 m-auto rounded-full overflow-hidden">
                    <img
                      ...
                      src={`${process.env.ADMINURL}/uploads/blogs/${item?.thumbnail_url}`}
                      alt={`${process.env.ADMINURL}/uploads/blogs/${item?.thumbnail_url}`}
                    />
                  </div>
